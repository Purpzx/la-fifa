...
REFEREE: ${lineup.referee} — search avg cards/game`
    : `⚠️ LINEUPS NOT CONFIRMED — search for expected XI and injuries`;

  const matchInfo = `${match.home} vs ${match.away} | ${match.kickoff_et} ET | ${match.venue} (${stadium.outdoor?'OUTDOOR':'INDOOR'}: ${stadium.notes}) | ${match.stage}
${context}
${lineupText}`;
...
- Tournament stage: apply context given
...
RULES: Only confirmed starters for player props. No goalscorer props. Player: shots,passes,fouls,tackles,saves,crosses,cards. Game: corners,cards,BTTS,shots,handicap. Team: team corners,clean sheet,team shots,first corner. Best book across all 5. Confidence 5=lock(rare),4=strong,3=moderate. Exactly 2+2+2.`;
  ...
  const response = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: `${matchInfo}\n\nSearch for: referee avg cards/game, group standings, injury news, best odds across all 5 books, both teams' last WC match stats. Return JSON only.`
    }],
  });
...
export default async function handler(req, res) {
  ...
  for(const match of todayMatches) {
    ...
    // Check timing window: 60-90 min before this match's kickoff
    ...
    // Try to fetch confirmed lineup for this match
    const lineup = await fetchLineupForMatch(match);

    // If lineups not confirmed yet and we're more than 60 mins out, wait
    if(!lineup && minsUntil > 60) {
      results.push({match:`${match.home} vs ${match.away}`, skipped:true, reason:"Lineups not confirmed yet"});
      continue;
    }
